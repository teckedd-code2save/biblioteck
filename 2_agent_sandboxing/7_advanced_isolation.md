# 🔒 Article 6: Beyond Code: Sandboxing Files and Web Access
...
> [!TIP]
> **Real-World Example:** In the MCP (Model Context Protocol) servers we use, the `mcp_postgres_server` allows the agent to `add-note` and `summarize-notes`. The database stays persistent, while the agent's "thinking environment" remains ephemeral.
...
### 🗺️ Navigation
**Previous:** [Article 5: Building the Workbench](./6_building_the_workbench.md)  
**Next Chapter:** [Article 7: The Agentic Bridge](../3_integration_deployment/8_the_agentic_bridge.md)  
